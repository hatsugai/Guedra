...
let init wch pch cch nch range0 page0 index0 =
  ...
  and win_msg msg =
    match msg with
      Paint (x, y, w, h) ->
       ...
       (* thumb *)
       (if cc.range > 0 && cc.width > o_o.scroll_thumb_size then (
          set_color o_o.color_handle;
          fill_rect (flo cc.pos) 0.0 (flo o_o.scroll_thumb_size) cc.fHeight));
    ...
    | WinSize (w, h) ->
       ...
       let p = calc_pos w cc.range cc.index in
       cc.pos <- p;
       invalidate ()
    | MouseDown (x, y, state, button) ->
       ...
       if x < cc.pos then (
         ...
         let p = calc_pos cc.width cc.range i in
         cc.index <- i;
         cc.pos <- p;
         notify (Pos i);
         invalidate ())
       else if x >= cc.pos + o_o.scroll_thumb_size then (
         (* page down *)
         ...
         notify (Pos i);
         invalidate ())
       else if cc.width > o_o.scroll_thumb_size && cc.range > 0 then (
         (* thumb *)
         cc.capture <- true;
         cc.displacement <- cc.pos - x;
         process ())
       else
         process ()
    ...
    | MouseUp (x, y, state, button) ->
       if cc.capture then
         let p = max 0
                   (min (x + cc.displacement)
                      (cc.width - o_o.scroll_thumb_size))
         in
         let i = calc_index cc.width cc.range p in
         let p = calc_pos cc.width cc.range i in
         cc.pos <- p;
         cc.index <- i;
         cc.capture <- false;
  ...
  and win_cmd msg =
    match msg with
      SetIndex i ->
       if i >= 0 && i <= cc.range then
         (cc.index <- i;
          cc.pos <- calc_pos cc.width cc.range i;
          invalidate ())
       else
         process ()
    | SetRange (r, i) ->
