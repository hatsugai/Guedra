...
let init wch pch cch nch ud dc =
       ...
       send wch_hsp msg
         (fun () ->
           send cch_hsp (Splitter.SetBorder border)
             (fun () ->
               send cch_lb Checklist.QuerySize
                 (fun () ->
                   recv nch_lb always
                     (fun (_, ack) ->
                       match ack with
                         Checklist.AckSize (items, lines, pos) ->
  ...
  and win_cmd msg =
    match msg with
      Checklist.SetList (v, chkmap, sel) ->
       send cch_lb msg
         (fun () ->
           send cch_lb Checklist.QuerySize
             (fun () ->
    ...
    | Checklist.Select sel ->
       send cch_lb msg process
    | _ -> process ()

  and handle_lb (ch, msg) =     (* from Checklist *)
    match msg with
      Checklist.OnSelect (cs, sel) ->
       notify msg;
       process ()
    | Checklist.OnPosChanged pos ->
       notify msg;
       send cch_vs (Scroll.SetIndex pos) process
    | Checklist.OnCheck (cs, index, b_check) ->
       notify msg;
       process ()
  ...
  and handle_vs (ch, msg) =     (* from Vscroll *)
    match msg with
      Scroll.Pos pos ->
      send cch_lb (Checklist.SetPos pos) process
  ...
  par
    [ (fun () -> Hsplitter.init Splitter.Fixed wch_hsp rch cch_hsp nch_hsp rch_hsp
                   wch_vs wch_lb
                   200 0 0);
      (fun () -> Checklist.init wch_lb rch_hsp cch_lb nch_lb ud dc);
